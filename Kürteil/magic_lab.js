...
function castFireball() {
  ...
  // Start am Tip des Stabes
  tip.getWorldPosition(start);
  ball.position.copy(start);

  // Flugrichtung: von dort aus in Kamerarichtung
...
function castLightning() {
  ...
  const bolt = new THREE.Mesh(
    new THREE.TubeGeometry(curve, 12, 0.06, 8, false),
    new THREE.MeshBasicMaterial({ color: 0x99eeff, transparent: true, opacity: 0.9 })
  );
  scene.add(bolt);

  setTimeout(() => scene.remove(bolt), 150);

  // Blitz trifft bis zu 3 Bücher im Radius
  const radius = 4;
  const targets = boxes.filter(b => b.mesh.position.distanceTo(end) < radius).slice(0, 3);
  targets.forEach((b, i) => {
    createExplosion(b.mesh.position.clone());
    breakBox(boxes.indexOf(b));
  });

  // Optional: Bildschirm-Flash (z. B. mit CSS-Overlay)
}
...
function animate(time) {
  ...
  // Langsamer drehender Zauberstab
